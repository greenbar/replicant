...
public class MethodMocker1Test extends TestCase {

  private static final Mock         MOCK          = new Mock();
  private static final String       METHOD        = "method";
  private static final A            ARG1          = new A(2);
  private static final List<Object> ARGUMENTS     = list((Object)ARG1);
  ...
  private MethodMocker1<X, A>     methodMocker;

  public void setUp() throws Exception {
    baseMethodMocker = new MockBaseMethodMocker<X>(MOCK, METHOD);
    methodMocker    = new MethodMocker1<X, A>(MOCK, METHOD, baseMethodMocker);
  }
  
  public void testExpect() throws Exception {
    baseMethodMocker.onCallToExpectReturn(RESPONSE);
    
    Response<X> response = methodMocker.expect(ARG1);
  ...
  public void testCall() throws Exception {
    ...
    X returnValue = methodMocker.call(ARG1);
  ...
  public void testAssertCalled() throws Exception {
    ...
    try {
      methodMocker.assertCalled(ARG1);
  ...
  public void testAssertNotCalledWithArguments() throws Exception {
    RuntimeException exception = new RuntimeException("test");
    baseMethodMocker.onCallToAssertNotCalledWithArgumentsThrow(exception);
    
    try {
      methodMocker.assertNotCalled(ARG1);
      fail("Expected an exception.");
    } catch (RuntimeException e) {
      assertSame(exception, e);
    }
    
    baseMethodMocker.assertCalledAssertNotCalledWithArguments(ARGUMENTS);
  }
  ...
  public void testEquality() throws Exception {
    ...
    MethodMocker1<X, A> mocker = new MethodMocker1<X, A>(MOCK, METHOD, baseMethodMocker1);
    
    assertEquals(false, mocker.equals(null));
    assertEquals(false, mocker.equals("not a MethodMocker1"));
    assertEquals(false, mocker.equals(new MethodMocker1<X, A>(new Mock(), METHOD,  baseMethodMocker1)));
    assertEquals(false, mocker.equals(new MethodMocker1<X, A>(MOCK,       "other", baseMethodMocker1)));
    assertEquals(false, mocker.equals(new MethodMocker1<X, A>(MOCK,       METHOD,  baseMethodMocker2)));
